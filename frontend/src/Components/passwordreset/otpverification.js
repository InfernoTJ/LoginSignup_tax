...
<Typography variant='h6' sx={{fontSize:'15px', mb:8 }}>{formData.email}</Typography>
    <OTP value={otp} onChange={setOtp} length={5} />
    <Typography variant='h6'  sx={{ fontSize:'15px', mb:2 , mt:2 , color:'grey'}} >Didnt receive it? <Button onClick={resendotp} sx={{ ":hover":{ bgcolor:'transparent'}}}  >Resend Code</Button></Typography>
  </Box>
  )
})
